'use strict';
// キリシマ N人エンジン（盤・合法手・閉領域塗り・UCT MCTS）。trends.js / selfplay.js が共有。
...
const C=Math.SQRT2;
function rollout(s0,cells){const s=clone(s0);while(s.turn!==0){const n=legal(s,s.turn,_buf);if(!n){advance(s);continue;}play(s,_buf[(_rng()*n)|0],s.turn);}const r=new Float64Array(s.P+1);for(let i=1;i<=s.P;i++)r[i]=s.score[i]/cells;return r;}
...
function best(root,iters){const P=root.P,cells=root.M*root.M;const rt=node(clone(root));for(let it=0;it<iters;it++){let nd=rt;const path=[nd];while(nd.u.length===0&&nd.c.length>0){nd=sel(nd);path.push(nd);}if(nd.u.length>0&&nd.s.turn!==0){const id=nd.u.pop();const ns=clone(nd.s);play(ns,id,nd.s.turn);const ch=node(ns);ch.m=id;nd.c.push(ch);path.push(ch);nd=ch;}const r=nd.s.turn===0?term(nd.s,cells):rollout(nd.s,cells);for(const x of path){x.n++;for(let p=1;p<=P;p++)x.W[p]+=r[p];}}let bm=-1,bv=-1;for(const ch of rt.c)if(ch.n>bv){bv=ch.n;bm=ch.m;}return bm;}
...
if(typeof module!=='undefined'&&module.exports) module.exports=__api;        // Node（selfplay/ladder/trends/probe）
